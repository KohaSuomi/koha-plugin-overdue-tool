...
sub after_circ_action {
    ...
    # 3) Check if the patron still has overdue invoiced items.
    #    If yes, keep the debarment — those items may also need invoicing.
    my $sth_overdue_invoiced = $dbh->prepare(
        "SELECT 1 FROM issues JOIN items ON issues.itemnumber = items.itemnumber WHERE issues.borrowernumber = ? AND items.notforloan = ? AND issues.date_due < NOW() LIMIT 1"
    );
    $sth_overdue_invoiced->execute($borrowernumber, $invoicedstatus);
    return if $sth_overdue_invoiced->fetchrow_array();

...
subtest 'invoiced item with debarment but patron has overdue non-invoiced loans - debarment removed' => sub {
    plan tests => 2;
    ...
    ok(!$sth->fetchrow_array(), 'debarment removed despite non-invoiced overdue loans');

    $schema->storage->txn_rollback;
};

subtest 'invoiced item with debarment but patron has other non-overdue invoiced loans - debarment removed' => sub {
    plan tests => 2;
    ...
    $sth->execute($debarment_id);
    ok(!$sth->fetchrow_array(), 'debarment removed despite other non-overdue invoiced loans');

    $schema->storage->txn_rollback;
};

subtest 'invoiced item with debarment but patron has overdue invoiced loans - debarment stays' => sub {
    plan tests => 1;
    ...
    my $overdue_invoiced_item = _create_item(
        homebranch => $patron->branchcode,
        notforloan => 6,
    );
    _create_debarment(
    ...
    _create_issue(
        borrowernumber => $patron->id,
        itemnumber     => $overdue_invoiced_item->itemnumber,
        date_due       => '2020-01-01 00:00:00',
        branchcode     => $patron->branchcode,
    );
    ...
    ok(!$result, 'overdue invoiced loans remain, debarment stays');
